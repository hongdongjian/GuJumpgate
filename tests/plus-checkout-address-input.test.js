      ...
      if (url === 'https://chatgpt.com/backend-api/payments/checkout') {
        return {
          ...
          json: async () => ({
            ...
            url: `https://pay.openai.com/c/pay/hosted_${checkoutSessionId}`,
          }),
        };
      }
...
test('CREATE_PLUS_CHECKOUT keeps PayPal internal flow but defaults billing to US/USD and prefers real hosted checkout', async () => {
  ...
  assert.equal(result.preferredCheckoutUrl, 'https://pay.openai.com/c/pay/hosted_cs_paypal');
...
test('CREATE_PLUS_CHECKOUT uses ID/IDR and openai_llc merchant path for GoPay', async () => {
  ...
  assert.equal(result.chatgptCheckoutUrl, 'https://chatgpt.com/checkout/openai_llc/cs_gopay');
  assert.equal(result.checkoutSessionId, 'cs_gopay');
  assert.equal(result.processorEntity, 'openai_llc');
  assert.equal(result.hostedCheckoutUrl, 'https://pay.openai.com/c/pay/hosted_cs_gopay');
  assert.equal(result.convertedCheckoutUrl, 'https://chatgpt.com/checkout/openai_llc/cs_gopay');
  assert.equal(result.preferredCheckoutUrl, 'https://chatgpt.com/checkout/openai_llc/cs_gopay');
